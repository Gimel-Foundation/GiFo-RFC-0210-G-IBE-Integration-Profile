...
- Appendix AK - SAP Cloud Identity Services + SAP Joule Agent Identity Cookbook (Preliminary)
...
SAP Cloud Identity Services + SAP Joule (Appendix AK, Preliminary);
...
**Preliminary status.** This appendix is **Preliminary** and is authored entirely from publicly-available SAP materials: the SAP Cloud Identity Services documentation set on SAP Help Portal (Identity Authentication, Identity Provisioning, Identity Directory), the SAP Authorization and Trust Management Service (XSUAA) documentation for SAP BTP, the SAP Audit Log Service documentation, SAP’s public Joule agent announcements (SAP Sapphire and SAP TechEd cycles, 2024–2026, including the Joule Agents and Joule Studio announcement register), and SAP’s public Business AI / SAP Knowledge Graph communications. 
**Every architectural claim in this appendix is sourced to public documentation and is marked as inferred where the public record is silent; no platform characterisation is original analysis without an attributed source.** A §AK.7 *Vendor-Acknowledgment-Update* placeholder is reserved for SAP architectural publication, for any SAP-Foundation collaboration pass, and for the SAP Sapphire / TechEd announcement windows. Per §13.7 of the body, this cookbook slot is flagged *subject to per-connector planning and IAM-vendor collaboration*; the intended closure path for this appendix is a vendor-verified pass with SAP.

The appendix follows the §F template — six normative sub-sections (§AK.1 Scope and Posture; §AK.2 Three-Service Mapping; §AK.3 Boundary-Primitive Mapping; §AK.4 NCI / NCP / NCO / NCM Frequency Observations; §AK.5 Bridge-Contract Mapping; §AK.6 Conformance Profile Recommendation) plus the reserved §AK.7 *Vendor-Acknowledgment-Update* placeholder.

AK.1 Scope and Posture
~~~~~~~~~~~~~~~~~~~~~~

SAP operates the archetype-(vii) *enterprise-application-suite IAM* pattern per §13 of the body: the identity directory of record is the enterprise application estate itself rather than a standalone workforce IDP. The relevant public surfaces are:

-  **SAP Cloud Identity Services** — the umbrella for (i) **Identity Authentication (IAS)**, the authentication and SSO service (SAML 2.0 / OpenID Connect front channel; risk-based authentication policies); (ii) **Identity Provisioning (IPS)**, the provisioning engine synchronising identities across SAP and non-SAP target systems (SCIM 2.0-based transformation pipelines); and (iii) the **Identity Directory**, the SCIM 2.0 user/group store that anchors the SAP-side identity object.
-  **SAP Authorization and Trust Management Service (XSUAA)** on SAP BTP — the OAuth 2.0 authorization server issuing platform tokens; authority is encoded as role collections, role templates, and scopes bound to the identity at token issuance.
-  **The SAP application-layer authorization heritage** — the ABAP authorization-object / PFCG-role engine in S/4HANA and the business-object-level authorization profiles that remain the operative Runtime-PEP inside the application estate.
-  **SAP Audit Log Service** on SAP BTP — the platform-wide audit-event sink.
-  **SAP Joule** — SAP’s generative-AI copilot, extended per SAP’s public announcements into **Joule Agents** (autonomous / semi-autonomous agents operating across SAP business processes) with **Joule Studio** as the agent-building surface, and the **SAP Knowledge Graph** as the semantic grounding layer supplying business-data and business-process context to AI scenarios (not part of the authentication or authorization surface).

The architectural posture is *identity = mandate* per §1.3 of the body, in its enterprise-application-suite form: on the public-record evidence, a Joule agent’s authority to act is encoded as the joint state of (a) the SAP user master record it is anchored to (with an agent-class flag, per the §13.7 sketch — the precise Identity Directory schema attribute is not publicly disclosed and is marked inferred), (b) the role collections / scopes issued to it at the XSUAA token boundary, and (c) the business-object-level authorization profile evaluated inside the application at action time. There is no separate principal-signed mandate document riding with the agent’s request in the public record — authority lives in the identity object and its platform-bound guardrails (scoping and access control), not in a per-request auditable Power-of-Attorney credential. The structural consequence, per the body’s Phase-1 / Phase-2 framing: SAP’s surface supplies a well-instrumented **Phase 2** (identity, authentication, platform-bound authorization) while **Phase 1** (the credential-bound authority layer per RFC 0115 + RFC 0117 §4 — auditable mandating at request level) is not native and Must be supplied via the §6 bridge contract. The step from platform-bound guardrails to native PoA credentials is precisely the seam this cookbook instruments.

The posture of this appendix is *catalogue-and-prescribe* per §3.0 of the body. SAP’s is a coherent identity-bound architecture with an unusually deep application-side authorization heritage (the ABAP authorization-object model predates the modern IAM industry). SAP also operates a **semantic grounding layer (SAP Knowledge Graph)** whose documented purpose is supplying business-data and business-process context to AI scenarios; on the public record it plays **no role in SAP’s authentication or authorization surface** and is referenced in this appendix only as an optional Foundation-side data source (see §AK.5). The mapping below names the CCPE-G boundary primitives onto the SAP Phase-2 surface as observed in the public record, identifies where Phase 1 is missing, and specifies how the bridge contract of §6 supplies it.

**Foundation posture toward SAP as a deployer.** This appendix is authored under the *catalogue-and-prescribe* discipline as a deployer-class catalogue entry from public documentation; no claim is made against SAP as vendor beyond what the public record supports. The §13.7 flag *subject to IAM-vendor collaboration* applies: the natural closure path is a jointly-verified pass, and the §AK.7 placeholder is reserved for it.

AK.2 Three-Service Mapping
~~~~~~~~~~~~~~~~~~~~~~~~~~

+-----------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| CCPE-G Service (per §3.2)         | SAP realisation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
+===================================+=======================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================+
| **Directory**                     | SAP Cloud Identity Services **Identity Directory** (SCIM 2.0 user/group store), federated by **Identity Provisioning** into the application-estate user masters (S/4HANA SU01 user master, SuccessFactors employee record, etc.). The Joule agent identity is anchored at the SAP user master with an agent-class designation per the §13.7 sketch *(schema attribute inferred — not publicly disclosed)*.                                                                                                                                            |
+-----------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Authority-Surface**             | SAP Cloud Identity Services admin console + SCIM 2.0 API of the Identity Directory; Identity Provisioning transformation pipelines; **XSUAA** role-collection assignment surface on SAP BTP; the application-side PFCG-role / authorization-object maintenance surface inside the estate. Joule Studio is the emerging agent-definition surface *(its authority-assignment semantics are not publicly specified; inferred to bind agent capabilities to role collections at deployment time)*.                                                        |
+-----------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| **Runtime-PEP**                   | Two-layered: (i) **XSUAA at token issuance** (scopes / role collections evaluated when the agent obtains its platform token); (ii) the **application-layer authorization check** (ABAP authorization objects; business-object-level authorization profiles) at action execution. The SAP Knowledge Graph plays no role in this decision path on the public record. The natural Identity-PEP is the layered Foundation-side wrapper translating the XSUAA + application-layer decision outputs into the RFC 0117 PERMIT / DENY / CONSTRAIN vocabulary. |
+-----------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+

The audit sink is the **SAP Audit Log Service** (BTP-side) together with the application-layer change documents and security audit log (estate-side) — a split audit surface that is itself a §AK.4 consideration.

AK.3 Boundary-Primitive Mapping
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

+-----------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| CCPE-G Boundary Primitive         | SAP realisation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
+===================================+============================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================================+
| ``identity_issuance``             | Identity Directory SCIM 2.0 ``POST /Users`` (agent-class designation per §AK.2, attribute shape inferred); Identity Provisioning propagation into the estate user masters; XSUAA role-collection assignment at issuance encodes the initial platform scope; application-side PFCG-role assignment encodes the estate-side scope. For Joule Agents, issuance is inferred to occur at Joule Studio deployment time; the public record does not disclose whether per-agent-instance or per-agent-class identities are issued. |
+-----------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| ``identity_attribute_mutation``   | Identity Directory SCIM 2.0 ``PATCH /Users/{id}``; role-collection assignment changes at the XSUAA surface; PFCG-role and authorization-object changes estate-side (captured in application change documents). Per-attribute decomposition is available in the Audit Log Service for BTP-side writes and in change documents for estate-side writes — but across **two separately-queried audit surfaces** (see NCP observation, §AK.4).                                                                                   |
+-----------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| ``identity_revocation``           | Identity Directory SCIM 2.0 ``DELETE /Users/{id}`` for hard revocation; user-lock semantics (estate-side SU01 lock; IAS user deactivation) for soft revocation; role-collection unassignment for scope-narrowing revocation. Revocation propagation across the estate rides the Identity Provisioning synchronisation cycle — meaning revocation latency is bounded by provisioning-job cadence unless real-time provisioning is configured *(a Strict-profile freshness consideration)*.                                  |
+-----------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| ``identity_introspection``        | Identity Directory SCIM 2.0 ``GET /Users/{id}``; XSUAA token introspection at the platform boundary; estate-side SU01 / authorization-trace surfaces. The ``directory_etag`` analogue is the SCIM 2.0 ``meta.version`` / ``meta.lastModified`` on the Identity Directory resource.                                                                                                                                                                                                                                         |
+-----------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+

AK.4 NCI / NCP / NCO / NCM Frequency Observations
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Sourced from the SAP Help Portal documentation set and SAP’s public Joule announcements; **Preliminary — all frequency observations are inferences from the documented deployment patterns, subject to vendor verification at §AK.7.**

-  **NCO is the expected dominant shape.** SAP identity administration is heritage-deep administrator-side (SU01, PFCG, IPS-managed provisioning): Joule agent identities are, on the documented pattern, administrator-issued on behalf of business users or business roles rather than issued from a recorded principal-delegation event. This is consistent with the enterprise-application-suite archetype and fit for purpose for non-fiduciary action classes.
-  **NCM is structurally likely for suite-wide Joule deployments.** A Joule agent deployed tenant-wide, acting for many business users across a process (the documented Joule pattern), falls into NCM unless a per-user agent-identity-issuance flow is configured. The user-master anchoring gives SAP an unusually natural remediation path — the estate already maintains per-user records at fine granularity — but the public record does not document a per-user agent-instance issuance flow for Joule.
-  **NCP is the distinctive SAP-specific risk — the split audit surface.** BTP-side events (Audit Log Service) and estate-side events (change documents, security audit log) are separately captured and separately queried; the per-action reconstruction the §10 audit invariants require is only achievable when both surfaces are consolidated. Deployments that consume only the BTP-side surface will be NCP against estate-side authority mutations, and vice versa.
-  **NCI is rare where the user-master anchoring is used as designed.** The agent-anchored-at-user-master pattern natively carries a principal-relationship (the anchoring itself); NCI would arise mainly in deployments that anchor agents at technical/service users disconnected from any acting-on-behalf-of principal — a documented anti-pattern with communication users that SAP’s own security guidance discourages.

**Combinations.** Suite-wide Joule deployments are naturally NCO + NCM together, with NCP as the audit-consolidation risk on top; the §3.4.5 worst-case-declaration note applies.

AK.5 Bridge Contract Mapping
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The bridge contract of §6 maps onto the SAP surface as follows:

-  The **synthetic mandate envelope** is derived by a Foundation-side bridge service that consumes (a) Identity Directory SCIM change events and Identity Provisioning job results (for ``identity_issuance`` / ``identity_attribute_mutation``); (b) Audit Log Service event streams (BTP-side authority mutations); and (c) estate-side change-document extracts for PFCG/authorization-object changes. The bridge is **naturally triggered at the SAP role-assignment boundary** per the §13.7 sketch: a role-collection or PFCG-role assignment to an agent-anchored user master is the platform-native event most closely corresponding to a mandate-scope grant.
-  The **synthetic mandate’s ``scope`` field** naturally consumes the SAP role + business-object-level authorization profile per §13.7 — i.e., the union of XSUAA scopes and the estate-side authorization-object values resolved for the agent’s user master. *(Prospective — inferred, not documented SAP functionality.)* The **SAP Knowledge Graph** could additionally serve as a Foundation-side enrichment source for this projection: where the graph relates business objects to their processes, the bridge could annotate the projected ``scope`` with business-process context rather than emitting a flat permission list. To be precise about the facts: on the public record the Knowledge Graph’s documented purpose is **semantic grounding of business data and processes** (supplying AI agents with business context) — it plays **no role** in SAP’s authentication flow (IAS with OAuth 2.0 / OIDC / SAML) or in SAP’s authorization decision path (XSUAA + ABAP authorization objects, both deterministic), and whether its API surface is suitable for this enrichment is itself a §AK.7 verification item. Any such consumption would be **Foundation-side and read-only**; nothing in this mapping asserts or requires that SAP evaluates the graph at its enforcement points. The authoritative inputs to the ``scope`` projection remain the XSUAA and authorization-object surfaces alone.
-  The **bridge boundary trigger conditions** of §6.2 map to: (a) sub-agent actions where a Joule agent invokes a narrower-tasked agent (the Joule Studio multi-agent pattern per SAP’s public announcements); (b) cross-platform actions where the downstream is non-SAP (Microsoft Entra, Okta, AWS/GCP IAM, third-party APIs invoked from BTP); (c) commerce-class actions per RFC 0170 (procurement, ordering, and payment-adjacent process steps inside S/4HANA are the natural commerce-class sites); (d) downstream-consumer-requested envelopes via the ``Accept: application/vnd.gimel.gauth.poa+jwt`` header.
-  The **bridge-emission audit event** is emitted into the Audit Log Service pipeline alongside the source events, with the ``bridge_evidence`` field referencing the SCIM ``meta.version`` of the Identity Directory snapshot used, thereby also consolidating the split audit surface (the NCP remediation of §AK.4).

Two further bridge usage patterns would become available **if** the prospective Knowledge Graph enrichment above is realised — both are *(prospective)* **applications of already-specified Foundation mechanisms**, not new architecture and not documented SAP functionality:

-  **PoA-narrowing verification (application of RFC 0115 §3.7).** The mechanism is the standard **PoA narrowing** condition of RFC 0115 §3.7 — each link of a delegation chain may only narrow, never widen — applied unchanged; the Knowledge Graph is merely one projection surface it acts upon, not an SAP-specific variant of the mechanism. Where both the parent mandate’s ``scope`` and a sub-agent’s requested ``scope`` are projected onto the Knowledge Graph (per the semantic projection above), the narrowing condition becomes machine-decidable as **subgraph containment**: the sub-agent’s projected scope Must be a subgraph of the parent’s projected scope. PoA narrowing then constrains, along the chain and traceably back to the originating Power of Attorney and the human principal who issued it, like: **what** the agent may do with that data, **which** decisions and actions it may trigger, and **which** commitments (including commerce-class actions per RFC 0170) it may enter into. This replaces string-level comparison of flat permission lists, which is not cleanly decidable across the heterogeneous SAP permission model (XSUAA scopes vs estate-side authorization-object values), and makes the §6.2(a) sub-agent trigger condition verifiable at chain-validation time rather than by manual review.
-  **PoA-compliance evaluation over the process graph (application of the Foundation’s published PoA-compliance monitoring).** Where the mandated ``scope`` is graph-projected, the agent’s actual action trace (reconstructed from the consolidated audit surface per the NCP remediation above) can be evaluated against the mandated **process subgraph**: an agent acting *within* its platform-side technical permissions but *outside* its mandated process context is detectable. This is precisely the class of deviation a Phase-2-only surface cannot see — the platform evaluates *authorised / not authorised*, never *mandated / not mandated* — and it operationalises PoA-compliance monitoring at business-process granularity on the SAP estate.

**Determinism requirement (Normative).** All bridge-side consumption of the Knowledge Graph named in this section — the ``scope`` projection, the subgraph-containment narrowing check, and the process-subgraph PoA-compliance evaluation — Must Be performed by **deterministic graph evaluation** (graph traversal, lookup, and containment algorithms with reproducible results for identical inputs). AI-model-assisted or otherwise non-deterministic evaluation of authorisation state, guardrails, or mandate compliance is **excluded of the licensing of this RFC**, consistent with the Foundation’s licensing posture that AI-enabled Governance is a separate commercial-license domain (see the Legal Notice). This is a **license boundary, not a feasibility judgment**: probabilistic AI-based governance evaluation can be operated in an auditable manner, but its probabilistic character requires **additional risk-mitigation controls** beyond the deterministic reproducibility the §10 audit invariants assume for the open-specification surface — which is precisely why that domain is carried under the separate AI-enabled-Governance license scope with its own disclosure regime rather than in this open cookbook. Should the SAP platform itself introduce AI-assisted evaluation of its Phase-2 guardrails, that would be a platform-side property to be catalogued (not endorsed) at §AK.7 verification — it would not alter the deterministic obligation on the Phase-1 bridge side within this RFC’s open surface.

**Classification note — semantic evaluation does not change the scenario class.** Modern relationship-based access control (ReBAC) surfaces — Google Zanzibar and its consistency tokens (“zookies”), Auth0 FGA (Appendix B), and comparable relationship-tuple engines — remain **§3.0a.1 Scenario S1** surfaces: platform-bound, hook-driven guardrails evaluated at the platform’s own enforcement points against the platform’s own relationship store. The same holds for the SAP guardrail surface (XSUAA + ABAP authorization objects) — and it would hold equally if SAP were in the future to bring the Knowledge Graph into its authorization path. Semantic or relationship-typed evaluation raises the *quality* of the Phase-2 decision; it does not change its *nature* — no principal-signed, request-level authority record exists, and the S1 conformance ceiling of §11.1a applies unchanged until the §6 bridge supplies Phase 1. Note also that S1 has no narrowing condition at all, because it has no chain: each agent identity (including a sub-agent’s) receives its platform-bound permissions independently, so a sub-agent holding *wider* platform permissions than its invoking agent is structurally undetectable in S1 — delegation-based escalation only becomes checkable once the §6 bridge materialises the chain as an artefact and the RFC 0115 §3.7 narrowing condition can attach to it.

AK.6 Conformance Profile Recommendation for SAP Cloud Identity + Joule
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Per the Phase-1 / Phase-2 framing of §3.0, the SAP surface as observed in the public record supplies **Phase 2 only** — and supplies it deeply: directory, provisioning, token issuance, and a business-object-granular application-side authorization engine older and finer-grained than most of the IAM industry. **Phase 1 — auditable, principal-signed mandating at request level — is not native and Must be supplied via the §6 bridge contract** mapped per §AK.5. In the vocabulary used in Foundation-SAP communications: the bridge is the step from *platform-bound guardrails* (scoping and access control encoded on the identity object) to *native Power-of-Attorney credentials* (audit-fest mandating at the request level), and it is additive — nothing in the SAP Phase-2 surface needs to change for the bridge to operate in front of it.

A Joule deployment **without** a §6-conformant bridge Should declare conformance only against the **Minimal** profile of §8.1 (Phase 2 only; permits NCI / NCP / NCO / NCM). A deployment that **does** run the §6-conformant bridge — emitting synthetic mandate envelopes at the §6.2 trigger conditions with the §6.4 audit discipline, consolidating the split audit surface per §AK.5 — Should declare **Standard**, or **Platform-Native** (binding to SAP’s published Cloud Identity + XSUAA guarantees as the Phase-2 contract, Phase 1 supplied by the bridge). The **Strict** profile is gated on two SAP-side properties not confirmed in the public record: (i) a sender-constrained credential shape at the agent-action boundary (the XSUAA token shape at the Joule agent boundary is not publicly specified; bearer is presumed), and (ii) revocation-propagation latency bounds tighter than the Identity Provisioning job cadence (real-time provisioning configuration). Both are natural §AK.7 verification items.

AK.7 Vendor-Acknowledgment-Update *(Reserved)*
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Reserved for: (a) SAP architectural publication on the Joule agent-identity object schema and agent-boundary credential shape; (b) the per-connector planning and IAM-vendor collaboration pass flagged at §13.7, including verification of the inferred items marked in §AK.1–§AK.6 (agent-class attribute shape, Joule Studio authority-assignment semantics, per-agent-instance vs per-agent-class issuance, XSUAA token shape at the agent boundary, revocation-propagation latency); (c) the SAP Sapphire / SAP TechEd announcement windows; (d) disposition of the Strict-profile gates named at §AK.6.
